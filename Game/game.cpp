...
void Game::save(World & world, const std::string& filename) {
    namespace sf = std::filesystem;

    std::ofstream outFile("./../resources/saves/" + filename);
    if (outFile.is_open()) {
...
void Game::loadSave(const std::string& path, World& world){
...
Room* Game::createRoom(const std::string& data){
...
Tile* Game::createTile(const std::string& data){
    std::stringstream ss(data);
    int  tx, ty;
    float px, py;
    bool isPass, isExit;
...
Enemy* Game::createEnemy(const std::string& data){
        std::stringstream ss(data);
        ...
        switch(eType){
            case CASUAL:
                return new Casual( sf::Vector2f(tx,ty),sf::Vector2f (px,py),sf::Vector2f(vx,vy),shootTimer,isActive, hearts, EnemyType::CASUAL,sf::Vector2f(psx, psy) );
            case SPEEDY:
                return new Speedy( sf::Vector2f(tx,ty),sf::Vector2f (px,py),sf::Vector2f(vx,vy),shootTimer,isActive, hearts, EnemyType::SPEEDY,sf::Vector2f(psx, psy));
            case FATMAN:
                return new Fatman( sf::Vector2f(tx,ty),sf::Vector2f (px,py),sf::Vector2f(vx,vy),shootTimer,isActive, hearts, EnemyType::FATMAN,sf::Vector2f(psx, psy));
            case BOSS:
                return new Boss( sf::Vector2f(tx,ty),sf::Vector2f (px,py),sf::Vector2f(vx,vy),shootTimer,isActive, hearts, EnemyType::BOSS,sf::Vector2f(psx, psy));
            default:
                return new Enemy("dem.png", sf::Vector2f(tx, ty), sf::Vector2f(px, py), sf::Vector2f(vx, vy),
                                 shootTimer, isActive, hearts, EnemyType::BOSS, sf::Vector2f(psx, psy), sf::Rect<float>(0,0,16,16));

        }

}

Character* Game::createPlayer(const std::string& data){
    std::stringstream ss(data);
    float vx,vy, shootTimer,px,py;
    int hearts, tx,ty;
    ss >> vx >> vy >>hearts >>shootTimer >>px >> py >> tx >> ty;
    return new Character( "player.png", sf::Vector2f(tx, ty), sf::Vector2f(px, py), sf::Vector2f(vx, vy), shootTimer, hearts);
}